...
describe("EtherDelta", () => {
    ...
    const etherDeltaJsonInterface = BaseContract_1.default.loadJsonInterfaceFromFile('./bin/contracts/EtherDelta');
    const etherDeltaContractBinary = BaseContract_1.default.loadBinaryFromFile('./bin/contracts/EtherDelta');
    ...
    const issuableTokenJsonInterface = BaseContract_1.default.loadJsonInterfaceFromFile('./bin/contracts/IssuableToken');
    const issuableTokenContractBinary = BaseContract_1.default.loadBinaryFromFile('./bin/contracts/IssuableToken');
    ...
    describe("Deploy contract", () => {
        test('with default arguments', async () => {
            expect.assertions(4);
            const txReceipt = await etherDelta.deployContract(etherDeltaContractOwner, undefined, feeAccount, 0, 3000000000000000);
    ...
    describe("deposit and withdrawls", () => {
        let etherDeltaContractAddress, test1TokenContractAddress, test2TokenContractAddress;
        beforeAll(async () => {
            const etherDeltaTxReceipt = await etherDelta.deployContract(etherDeltaContractOwner, undefined, feeAccount, 2000000000000000, 3000000000000000);
            etherDeltaContractAddress = etherDeltaTxReceipt.contractAddress;
            const token1TxReceipt = await issuableToken1.deployContract(token1Owner, undefined, "TST1", "Test 1 Token");
            ...
            const token2TxReceipt = await issuableToken2.deployContract(token2Owner, undefined, "TST2", "Test 2 Token");
        ...
        describe("orders", () => {
            ...
            test("trader 1 places an order", async () => {
                ...
                const txReceipt = await etherDelta.placeOrder(trader1, test1TokenContractAddress, new BN(11), test2TokenContractAddress, new BN(3), blockNumber + 100, ++orderNonce);
                expect(txReceipt.transactionHash).toHaveLength(66);
                expect(txReceipt.status).toEqual(1);
            ...
            test("trader 1 cancels the order", async () => {
                expect.assertions(15);
                const txReceipt = await etherDelta.cancelOrder(trader1, test1TokenContractAddress, new BN(11), test2TokenContractAddress, new BN(3), blockNumber + 100, orderNonce);
                expect(txReceipt.transactionHash).toHaveLength(66);
                expect(txReceipt.status).toEqual(1);
