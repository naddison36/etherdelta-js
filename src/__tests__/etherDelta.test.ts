...
import {ecsign, ecrecover, toBuffer, bufferToHex, bufferToInt} from "ethereumjs-util";
...
describe("EtherDelta", ()=>
{
    ...
    const issuableTokenJsonInterface = BaseContract.loadJsonInterfaceFromFile('./bin/contracts/IssuableToken.abi');
    const issuableTokenContractBinary = BaseContract.loadBinaryFromFile('./bin/contracts/IssuableToken.bin');
    ...
    describe("crypto", ()=>
    {
        beforeAll(async()=>
        {
            await etherDelta.deployContract(etherDeltaContractOwner, defaultGasLimit, defaultGasPrice,
        ...
        test("solidity sha256 of order params", ()=>
        {
            ...
            const shaHash = utils.soliditySha256(
        ...
        test("Ethereumjs-tx signing and recovering using secp256k1", async()=>
        {
            ...
            const messageBuffer = toBuffer('0x5965b604c8a3aeea98bac96fde194ce36307b45c97048484c9662ef3320baed2');
            ...
            const v = bufferToInt(signature.v);
            ...
            const recoveredAddressBuffer = ecrecover(messageBuffer, signature.v, signature.r, signature.s);
            ...
            const solidityRecoverredAddress = await etherDelta.ecrecover("0x19E7E376E7C213B7E7e7e46cc70A5dD086DAff2A", "0x5965b604c8a3aeea98bac96fde194ce36307b45c97048484c9662ef3320baed2",
                v, r, s);
